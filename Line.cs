...
namespace _3dgraphics
{
    internal class Line
    {
        ...
        public void Draw(SpriteBatch sb, Viewport vp, Utility.RenderModes renderMode)
        {
            Vector3 mid, diff;
            ...
            { 
                ...
                aXY *= Game1._perspectiveScale;
                bXY *= Game1._perspectiveScale;
                ...
                float minZ = -(halfScreen / Game1._perspectiveScale) * (float)Math.Sqrt(2);
                float maxZ = (halfScreen / Game1._perspectiveScale) * (float)Math.Sqrt(2);

                zOrder = (GetZ() - minZ) / (maxZ - minZ);
            }
            else
            {
                Vector2 aXY = new Vector2(-a.X, -a.Y - a.Z * (float)Math.Sin(Math.PI / 6));
                Vector2 bXY = new Vector2(-b.X, -b.Y - b.Z * (float)Math.Sin(Math.PI / 6));
                ...
                float halfHeight = Game1._screenHeight / 2;
                float minZ = -halfHeight / (Game1._perspectiveScale * (float)Math.Sin(Math.PI / 6));
                float maxZ = halfHeight / (Game1._perspectiveScale * (float)Math.Sin(Math.PI / 6));
